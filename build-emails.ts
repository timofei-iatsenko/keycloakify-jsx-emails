...
async function getTemplates(dirPath: string) {
  try {
    ...
    const files = items
      .filter(
        (item) => item.isFile() && !getBaseName(item.name).startsWith('_'),
      )
...
async function getThemes() {
  try {
    const raw: { id: string }[] = JSON.parse(
      await fs.readFile('./brands.json', 'utf8'),
    )

    return raw.map((brand) => 'brand-' + brand.id)
  } catch (err) {
    console.error('Error reading brands.json')
    throw err
  }
}
...
async function main() {
  ...
  const promises = tpls.map(async (file) => {
    const module = await (import(
      path.join(esbuildOutDir, getBaseName(file) + '.js')
    ) as Promise<{
  ...
  for (const locale of locales) {
    for (const themeName of themes) {
      const html = await render(
        createElement(Template, { themeName, locale }),
        {
          pretty: true,
        },
      )
